...
@Log4j
@Cmd(name = "update-sum", method = Method.POST, roles = {Role.ADMIN})
public class UpdateSumCmd implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            String sum = request.getParameter(Attribute.SUM);
            String tariffId = request.getParameter(Attribute.TARIFF_ID);

            service.updateSum(tariffId, sum);

...
@Log4j
@Cmd(name = "remove-usage", method = Method.POST, roles = {Role.USER, Role.ADMIN})
public class RemoveUsageCmd implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            service.deleteUsage(userId, tariffId);
...
//            log.warn("While adding tariff error occurred", e);
            CookieHelper.addCookie(Attribute.REMOVE_USAGE_ERROR, Attribute.OCCURRED, CookieHelper.UNDEFINED_AGE, response);
        }
